...
1. Doubles 默认写法: 123.5, 123.5e10
2. Floats 使用 f 或者 F 后缀：123.5f

```kotlin
fun main(args: Array<String>) {
    val number1 = 123.5
    val number2 = 123.5e10
    val number3 = 123.5f
    println(
        "${number1}\n${number2}\n${number3}\n"
    )
...
```kotlin
fun check(c: Char) {
    if (c == 1) { // 错误：类型不兼容，字符字面值用单引号括起来: '1'。
        // ……
    }
}
```
...
fun main(args: Array<String>) {
    //[0,2,4]
    val b = Array(3, { i -> (i * 2) })   // 类似于lambda表达式
    //读取数组内容
    println(a[0])    // 输出结果：1
    println(b[1])    // 输出结果：2
}
...
和Python一样，Kotlin 支持三个引号 """ 扩起来的字符串，支持多行字符串，比如：
...
`trimMargin`函数默认 | 用作边界前缀，但你可以选择其他字符并作为参数传入，比如 trimMargin(">")。
...
此外，Kotlin的字符串模板还可以是花括号扩起来的任意表达式:
